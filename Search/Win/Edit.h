...
{
public:
    StaticText(HWND winParent, int id) : SimpleCtrl(winParent, id) {}
};

class EditReadOnly : public SimpleCtrl
{
public:
    EditReadOnly(HWND winParent, int id) : SimpleCtrl(winParent, id) {}
...
{
public:
    Edit(HWND winParent, int id) : SimpleCtrl(winParent, id) {}
    ...
    int GetLen() const { return SendMessage(WM_GETTEXTLENGTH); }

    int GetLineCount() { return SendMessage(EM_GETLINECOUNT); }
    ...
    void Select() { SendMessage(EM_SETSEL, 0, -1); }
