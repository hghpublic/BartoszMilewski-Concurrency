...
Win::Maker::Maker(std::wstring const& className, HINSTANCE hInst)
    : _style(WS_OVERLAPPEDWINDOW), _exStyle(0), _className(className),
...
{
    HWND win = ::CreateWindowEx(_exStyle, _className.c_str(), title.c_str(),
                                _style, _x, _y, _width, _wineight, _winParent,
                                _menu, _winInst, &controller);
...
Win::ChildMaker::ChildMaker(std::wstring const& className, Win::Dow winParent,
                            int childId)
...
{
    HWND win =
        ::CreateWindowEx(_exStyle, _className.c_str(), text.c_str(), _style, _x,
                         _y, _width, _wineight, _winParent, _menu, _winInst, 0);
