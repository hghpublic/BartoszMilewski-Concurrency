...
public:
    ...
    // Line
    void LineTo(int x, int y) { ::LineTo(_windc, x, y); }

    void MoveTo(int x, int y) { ::MoveToEx(_windc, x, y, 0); }
...
{
public:
    PaintCanvas(HWND win) : Canvas(::BeginPaint(win, &_paint)), _win(win) {}

    ~PaintCanvas() { ::EndPaint(_win, &_paint); }
...
{
public:
    UpdateCanvas(HWND win) : Canvas(::GetDC(win)), _win(win) {}

    ~UpdateCanvas() { ::ReleaseDC(_win, _windc); }
...
{
public:
    DisplayCanvas() : Canvas(::CreateDC(L"DISPLAY", 0, 0, 0)) {}
};
} // namespace Win
