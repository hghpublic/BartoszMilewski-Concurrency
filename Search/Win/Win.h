...
public:
    ...
    void Destroy() { ::DestroyWindow(_win); }
    // Focus
    void SetFocus() { ::SetFocus(_win); }
    bool HasFocus() const { return ::GetFocus() == _win; }
    // Capture
    void CaptureMouse() { ::SetCapture(_win); }
    void ReleaseMouse()
    {
        if (HasCapture())
        {
    ...
    bool HasCapture() const { return ::GetCapture() == _win; }
    // Text/Caption
    ...
    // Window visibility
    void Show(int cmdShow = SW_SHOW) { ::ShowWindow(_win, cmdShow); }
    void Hide() { ::ShowWindow(_win, SW_HIDE); }
    void Update() { ::UpdateWindow(_win); }
    void Display(int cmdShow = SW_SHOW)
    {
        Show(cmdShow);
        Update();
    }
    void Enable() { ::EnableWindow(_win, TRUE); }
    void Disable() { ::EnableWindow(_win, FALSE); }
    // Moving
